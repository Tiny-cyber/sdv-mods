...
# 1. Install OpenSSH via MSI (fast, skips slow Windows Update)
$sshd = Get-Service sshd -ErrorAction SilentlyContinue
if ($sshd) {
    Write-Host "[1/3] SSH already installed" -ForegroundColor Green
} else {
    Write-Host "[1/3] Downloading OpenSSH..."
...
# 2. Add public key
Write-Host "[2/3] Configuring SSH key..."
...
# 3. Download bore and start tunnel
...
Write-Host "  Tell mian-ge the bore.pub:XXXXX below"
Write-Host "========================================" -ForegroundColor Cyan
Write-Host ""
